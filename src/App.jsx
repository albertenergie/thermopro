...
// Filet de sécurité : si l'activation du cache hors-ligne échoue pour une raison
// quelconque (navigation privée, plusieurs onglets ouverts, navigateur récalcitrant...),
// on retombe sur une connexion Firestore classique plutôt que de planter toute l'appli.
let db;
try {
  ...
  console.error("Cache hors-ligne indisponible, connexion Firestore standard :", e);
    ...
    <div className="modal-overlay"><div className="modal">
      ...
      <div className="form-grid">
        <div className="form-group full" style={{position:"relative"}}>
          <label>Client <span style={{fontWeight:400,color:"var(--muted)"}}>(laisser vide pour un rendez-vous personnel)</span></label>
      ...
      <div className="form-actions">
        <button className="btn btn-ghost" onClick={onClose}>Annuler</button>
        <button className="btn btn-primary" onClick={()=>onSave(f)} disabled={!f.date}>Enregistrer</button>
      </div>
    </div></div>
  );
}
                        ...
                        <div key={r.id} style={{margin:"4px 8px",background:isRealise?"#22c55e15":isConfirme?"#f9731615":"#f59e0b15",border:`1px solid ${isRealise?"var(--success)":isConfirme?"var(--accent)":"var(--warning)"}`,borderLeft:`4px solid ${isRealise?"var(--success)":isConfirme?"var(--accent)":"var(--warning)"}`,borderRadius:8,padding:"8px 12px"}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,flexWrap:"wrap"}}>
                            ...
                            <div style={{display:"flex",flexDirection:"column",gap:6,flexShrink:0,alignItems:"flex-end"}}>
                              ...
                              {!isRealise&&r.statut!=="Annulé"&&c&&<button className="btn btn-primary btn-sm" onClick={()=>openWizard(r)}>▶ Démarrer</button>}
